...
class StochasticRetirementEngine:
    ...
    def get_port_params(self, asset_key, override_port=None):
        if override_port: return PORTFOLIOS[override_port]['ret'], PORTFOLIOS[override_port]['vol']
        else: return PORTFOLIOS[self.inputs[asset_key]]['ret'], PORTFOLIOS[self.inputs[asset_key]]['vol']
    ...
    def setup_covariance_matrix(self, override_port=None):
        ...
        return np.linalg.cholesky(np.outer(vols, vols) * corr)
    ...
    def generate_stochastic_paths(self, L, seed=None, override_port=None):
        ...
        for yr in range(self.years):
            dW = correlated_shocks[:, yr, 0] * np.sqrt(dt)
    ...
    def run_mc(self, iwr, seed=None, roth_strategy=0, override_port=None):
        ...
        for yr in range(self.years):
            ...
            # Record terminal year snapshot at exact moment of death for objective function
            just_died_mask = (~household_alive) & (history['terminal_year'] == 0)
            history['terminal_year'][just_died_mask] = yr

        # Catch anyone who lived to 115 max cap
