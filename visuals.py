...
def plot_wealth_trajectory(history, target_floor, years_arr):
    ...
    fig.update_layout(title="Stochastic Portfolio Projections (In Today's Dollars)", hovermode="x unified", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_fan_chart(history, years_arr):
    ...
    fig.update_layout(title="Sequence of Return Risk (Fan Chart)", hovermode="x unified", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_liquidity_timeline(history, years_arr):
    ...
    fig.update_layout(title="Total Net Worth Forecast", hovermode="x unified", template="plotly_white")
    return fig
...
def plot_cash_flow_sources(history, years_arr):
    ...
    fig.update_layout(barmode='stack', title="Income Sources vs Total Spending Need", template="plotly_white")
    return fig
...
def plot_income_gap(history, years_arr):
    ...
    fig.update_layout(title="Income Gap Mapping", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_expenses_breakdown(history, years_arr):
    ...
    fig.update_layout(barmode='stack', title="Itemized Core Expenses", template="plotly_white")
    return fig
...
def plot_income_volatility(history, years_arr):
    ...
    fig.update_layout(title="Variable Spending: Guardrail Adaptive Cash Flows", hovermode="x unified", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_withdrawal_hierarchy(history, years_arr):
    ...
    fig.update_layout(barmode='stack', title="Dynamic Account Liquidation Hierarchy", template="plotly_white")
    return fig
...
def plot_taxes_and_rmds(history, years_arr):
    ...
    fig.update_layout(title="Tax Trajectory vs Mandatory RMD Cliffs", hovermode="x unified", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_legacy_breakdown(history):
    ...
    fig.update_layout(title="Terminal Estate Composition (At Life Expectancy)", template="plotly_white")
    return fig
...
def plot_roth_tax_impact(roth_results, winner, years_arr):
    ...
    fig.update_layout(title="Lifetime Tax Liability Comparison", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_ss_breakeven(ss_fra, age_arr, years_arr):
    ...
    fig.update_layout(title="Cumulative Guaranteed Income Breakeven Analysis", xaxis_title="Age", yaxis_title="", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig
...
def plot_medicare_comparison(history, years_arr, inputs):
    ...
    fig.update_layout(title="Lifetime Healthcare Cost Comparison", template="plotly_white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig