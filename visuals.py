...
def plot_net_spendable(history, years_arr):
    ...
    fig = px.bar(
        x=years_arr, 
        y=med_spendable, 
        labels={'x': 'Year', 'y': 'Net Income ($)'}, 
        title="Projected Post-Tax Real Income"
    )
...
def plot_liquidity_timeline(history, years_arr):
    ...
    fig.update_layout(
        title="Asset Liquidity & Composition Timeline", 
        xaxis_title="Year", 
        yaxis_title="Balance ($)", 
        hovermode="x unified"
    )
...
def plot_income_volatility(history, years_arr):
    ...
    fig.update_layout(
        title="Guyton-Klinger Guardrails: Spending Power Volatility", 
        xaxis_title="Year", 
        yaxis_title="Net Spendable Income ($)", 
        hovermode="x unified"
    )
    return fig
