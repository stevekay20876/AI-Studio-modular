...
def build_csv_dataframe(history, years_arr, age_arr, percentile=50):
    data = {
        ...
        "Annual Roth IRA Withdrawal": np.percentile(history['roth_withdrawal'], percentile, axis=0),    # ADDED
        "Annual Taxable Withdrawal": np.percentile(history['taxable_withdrawal'], percentile, axis=0),  # ADDED
        "Annual Cash/MM Withdrawal": np.percentile(history['cash_withdrawal'], percentile, axis=0),      # ADDED
